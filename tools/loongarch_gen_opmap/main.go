...
func main() {
	...
	// 4. Generate pseudo_opmap.rs file (pseudo-instructions)
	pseudoFile := filepath.Join(filepath.Dir(outputFile), "pseudo_opmap.rs")
	if err := generatePseudoOpmapFile(pseudoFile); err != nil {
		fmt.Printf("Error generating pseudo_opmap.rs: %v\n", err)
		os.Exit(1)
	}
}
...
// generatePseudoOpmapFile generates pseudo_opmap.rs with hand-coded pseudo-instruction entries.
// These entries use Double/Many templates with custom commands not found in the opcode database.
func generatePseudoOpmapFile(path string) error {
	file, err := os.Create(path)
	if err != nil {
		return err
	}
	...
	file.WriteString("// This file was generated by tools/loongarch_gen_opmap\n")
	file.WriteString("// Pseudo-instructions (not from opcode database, hand-coded)\n")
	file.WriteString("// DO NOT EDIT MANUALLY — changes will be overwritten by gen-opmap\n\n")
	file.WriteString("PseudoOps!(\n")

	// la = pcalau12i + addi.d (SPLIT_PCALA, Double template)
	file.WriteString("    \"la\" = [\n")
	file.WriteString("        Double(0x1A000000, 0x02C00000) ,\n")
	file.WriteString("        0x03 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [R, Offset] => [R(0), Repeat, R(5+32), Offset(SPLIT_PCALA)] ;\n")
	file.WriteString("    ]\n")

	// la.local = same as la
	file.WriteString("    \"la.local\" = [\n")
	file.WriteString("        Double(0x1A000000, 0x02C00000) ,\n")
	file.WriteString("        0x03 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [R, Offset] => [R(0), Repeat, R(5+32), Offset(SPLIT_PCALA)] ;\n")
	file.WriteString("    ]\n")

	// call36 = pcaddu18i + jirl (SPLIT_CALL36, $r1 hardcoded)
	file.WriteString("    \"call36\" = [\n")
	file.WriteString("        Double(0x1E000001, 0x4C000021) ,\n")
	file.WriteString("        0x02 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [Offset] => [Offset(SPLIT_CALL36)] ;\n")
	file.WriteString("    ]\n")

	// call30 = pcaddu12i + jirl (SPLIT_CALL30, $r1 hardcoded)
	file.WriteString("    \"call30\" = [\n")
	file.WriteString("        Double(0x1C000001, 0x4C000021) ,\n")
	file.WriteString("        0x03 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [Offset] => [Offset(SPLIT_CALL30)] ;\n")
	file.WriteString("    ]\n")

	// tail36 = pcaddu18i + jirl $r0 (SPLIT_CALL36, user supplies $rj)
	file.WriteString("    \"tail36\" = [\n")
	file.WriteString("        Double(0x1E000000, 0x4C000000) ,\n")
	file.WriteString("        0x02 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [R, Offset] => [R(0), Offset(SPLIT_CALL36), R(5+32)] ;\n")
	file.WriteString("    ]\n")

	// tail30 = pcaddu12i + jirl $r0 (SPLIT_CALL30, user supplies $rj)
	file.WriteString("    \"tail30\" = [\n")
	file.WriteString("        Double(0x1C000000, 0x4C000000) ,\n")
	file.WriteString("        0x03 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [R, Offset] => [R(0), Offset(SPLIT_CALL30), R(5+32)] ;\n")
	file.WriteString("    ]\n")

	// li.w = lu12i.w + ori (LiW32, compile-time immediate)
	file.WriteString("    \"li.w\" = [\n")
	file.WriteString("        Double(0x14000000, 0x03800000) ,\n")
	file.WriteString("        0x03 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [R, Imm] => [Rno0(0), Repeat, R(0+32), Repeat, R(5+32), LiW32] ;\n")
	file.WriteString("    ]\n")

	// li.d = lu12i.w + ori + lu32i.d + lu52i.d (LiD64, 64-bit immediate)
	file.WriteString("    \"li.d\" = [\n")
	file.WriteString("        Many(&[0x14000000, 0x03800000, 0x16000000, 0x03000000]) ,\n")
	file.WriteString("        0x02 , &[ExtensionFlags::Ex_BASE] ,\n")
	file.WriteString("        [R, Imm] => [Rno0(0), Repeat, R(0+32), Repeat, R(5+32), Repeat, R(0+64), Repeat, R(0+96), Repeat, R(5+96), LiD64] ;\n")
	file.WriteString("    ]\n")
